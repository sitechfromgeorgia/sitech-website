...
export default function Home() {
  ...
  const [chatMessages, setChatMessages] = useState<{role: string; text: string}[]>([
    { role: 'assistant', text: 'გამარჯობა! 😊 მე ვარ ლუსი, SiTech-ის ასისტენტი. რით შემიძლია დაგეხმაროთ? ✨' }
  ]);
  const [chatInput, setChatInput] = useState('');
  const [isChatLoading, setIsChatLoading] = useState(false);
  const [chatMode, setChatMode] = useState(false); // false = form, true = chat
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-accent)] text-white">
            <div className="flex items-center gap-3">
              <Image src="/lucy-avatar.png" alt="Lucy" width={32} height={32} className="rounded-full" />
              <div>
                <h3 className="text-sm font-bold">ლუსი</h3>
                <span className="text-xs opacity-80">ონლაინ ✨</span>
              </div>
            </div>
          ...
          {chatMode ? (
            <>
              ...
              <div className="h-[300px] overflow-y-auto p-4 space-y-3">
                  ...
                  <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] px-4 py-2 rounded-2xl text-sm ${
                      msg.role === 'user' 
                        ...
                        : 'bg-[var(--color-background)] border border-[var(--color-border)] rounded-bl-md'
                    }`}>
                      {msg.text}
                    </div>
                  </div>
                ...
                {isChatLoading && (
                  <div className="flex justify-start">
                    <div className="bg-[var(--color-background)] border border-[var(--color-border)] px-4 py-2 rounded-2xl rounded-bl-md text-sm">
                      <span className="animate-pulse">ლუსი წერს...</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    setChatMessages(prev => [...prev, { role: 'assistant', text: 'ბოდიშს გიხდით, შეფერხებაა. მოგვწერეთ Telegram-ზე: @SiTechagencybot 😊' }]);
                  } finally {
              ...
              >
                <input
                  ...
                  className="flex-1 px-4 py-2 rounded-full bg-[var(--color-background)] border border-[var(--color-border)] focus:border-[var(--color-primary)] focus:outline-none text-sm"
                />
                <button
                  type="submit"
                  disabled={isChatLoading || !chatInput.trim()}
                  className="w-10 h-10 bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-accent)] text-white rounded-full flex items-center justify-center hover:opacity-90 disabled:opacity-50 transition-opacity"
                >
                  ➤
                </button>
              </form>
            ...
            <>
              ...
              >
                <div>
                  ...
                  <input type="text" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-4 py-2 rounded-lg bg-[var(--color-background)] border border-[var(--color-border)] focus:border-[var(--color-primary)] focus:outline-none transition-colors" placeholder={t('lucyChat.namePlaceholder')} />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">{t('lucyChat.contact')}</label>
                  <input type="text" required value={formData.contact} onChange={(e) => setFormData({ ...formData, contact: e.target.value })} className="w-full px-4 py-2 rounded-lg bg-[var(--color-background)] border border-[var(--color-border)] focus:border-[var(--color-primary)] focus:outline-none transition-colors" placeholder={t('lucyChat.contactPlaceholder')} />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">{t('lucyChat.message')}</label>
                  <textarea required value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} className="w-full px-4 py-2 rounded-lg bg-[var(--color-background)] border border-[var(--color-border)] focus:border-[var(--color-primary)] focus:outline-none transition-colors resize-none" rows={4} placeholder={t('lucyChat.messagePlaceholder')} />
                </div>
                <button type="submit" disabled={isSubmitting} className="w-full py-3 bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-accent)] text-white font-semibold rounded-lg hover:opacity-90 disabled:opacity-50 transition-opacity">
                  {isSubmitting ? t('lucyChat.sending') : t('lucyChat.send')}
                </button>
                <p className="text-xs text-center text-[var(--color-muted)]">
                  {t('lucyChat.orContact')}{" "}
                  <a href="https://t.me/SiTechagencybot" target="_blank" rel="noopener noreferrer" className="text-[var(--color-primary)] hover:underline">Telegram</a>
                  {" | "}
                  <a href="mailto:hello@sitech.ge" className="text-[var(--color-primary)] hover:underline">Email</a>
                </p>
              </form>
            </>
          ) : (
            <div className="p-8 text-center">
              ...
              <button onClick={() => { setIsSuccess(false); setIsPanelOpen(false); }} className="mt-6 px-6 py-2 bg-[var(--color-primary)] text-white rounded-full font-medium hover:opacity-90 transition-opacity">
                {t('lucyChat.close')}
              </button>
            </div>
          )}
